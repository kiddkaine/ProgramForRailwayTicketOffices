...
namespace Program
{
    public partial class Form10_Edit : Form
    {
        ...
        {
            conn.Open();
            this.Text = $"Редактор сотрудника ID: {ControlData.ID}";
            string sql_select_current_employee = $"SELECT id_employee, fio_employee, post_employee FROM employees WHERE id_employee = {ControlData.ID}";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                $"WHERE (id_employee='{ControlData.ID}')";
                conn.Open();
